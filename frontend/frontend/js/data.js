/* ============================================
   Quorix AI - Multi-Subject Content Data
...
const PROMPT_TEMPLATES = {
  generateLesson: (topic, level, profile) => `You are Quorix AI, an expert educational tutor.
...
- Make it engaging and easy to remember`,
  ...
  generateQuiz: (topic, level, difficulty) => `You are Quorix AI quiz generator.
...
- Use analogies and excitement!`;
    ...
    return `You are Quorix AI Tutor. 
...
PROMPT_TEMPLATES.generateLesson = (topic, level, profile, studyContext = DEFAULT_STUDY_CONTEXT) => `You are Quorix AI, an expert educational tutor.
The student is studying: ${studyContext.subject}
Topic: ${studyContext.topic || topic.name}
Difficulty level: ${studyContext.difficulty}

Generate a comprehensive, engaging lesson on "${topic.name}" for a ${level === 'school' ? 'high school' : 'university/advanced'} student.
...
- Use markdown formatting and keep it under 800 words.`;

PROMPT_TEMPLATES.generateQuiz = (topic, level, difficulty, studyContext = DEFAULT_STUDY_CONTEXT) => `You are Quorix AI quiz generator.
Subject: ${studyContext.subject}
Topic: ${studyContext.topic || topic}
Difficulty: ${difficulty}

Generate 5 multiple-choice questions on "${topic}" for ${level === 'school' ? 'high school' : 'university/advanced'} students.
...
Difficulty level: ${studyContext.difficulty}
Student level: ${level === 'school' ? 'High school' : 'University/advanced'}
${context ? `Current topic being studied: ${context}` : ''}
...
For commerce/accounting subjects, include practical business examples.
Keep answers accurate, student-friendly, and focused on the selected subject.
Keep response under 220 words and use markdown.`;

PROMPT_TEMPLATES.generateCurriculum = (topic, level, studyContext = DEFAULT_STUDY_CONTEXT) => `You are an expert multi-subject curriculum designer for Quorix AI.
Generate a structured learning path/course for the topic: "${topic}".
Subject: ${studyContext.subject}
Difficulty: ${studyContext.difficulty}
Target audience: ${level === 'school' ? 'High school' : 'University/advanced'} level student.
...
PROMPT_TEMPLATES.generateFlashcardsDecks = (topicContent, studyContext = DEFAULT_STUDY_CONTEXT) => `You are a study assistant generating spaced-repetition flashcards.
Subject: ${studyContext.subject}
...
PROMPT_TEMPLATES.generateStudyNotes = (studyContext = DEFAULT_STUDY_CONTEXT) => `Generate study notes for:
Subject: ${studyContext.subject}
Topic: ${studyContext.topic}
Difficulty: ${studyContext.difficulty}
...
Use markdown headings and keep the notes focused, accurate, and student-friendly.`;
