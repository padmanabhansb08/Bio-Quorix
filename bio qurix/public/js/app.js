...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modelSelect) modelSelect.value = APP_STATE.ollamaModel;
...
async function generateLesson(topic) {
    ...
    try {
        ...
        const response = await callAI(prompt, APP_STATE.ollamaModel);
  ...
  `;
    ...
    if (ollamaModel) {
        try {
            ...
            response = await callAI(prompt, ollamaModel);
...
function setOllamaModel(model) {
    APP_STATE.ollamaModel = model;
    localStorage.setItem('bionexus_ollama_model', model);
    showToast(`Model set: ${model}`, 'sparkles');
}
    ...
    try {
        ...
        const response = await fetch('http://localhost:11434/api/generate', {
            ...
            body: JSON.stringify({
                ...
                stream: false,
                options: {
                    // Keep responses fast and predictable for a browser demo.
                    num_predict: 700
                }
            })
        });
...
2. The **${model}** model is installed (run \`ollama run ${model}\` in your terminal).
3. The server is accessible at \`http://localhost:11434\`.

${isTimeout ? `\n\n**Tip:** The request timed out after ${Math.round(timeoutMs / 1000)}s. Try a shorter prompt, or switch to a faster model in the top dropdown.` : ''}

*Error Details: ${err && err.message ? err.message : String(err)}*`;
    }
}
